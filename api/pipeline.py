...
import services.local_ner_client as local_ner_client
...
NNP_HANDLER = None  # gemini_client.GeminiHandler | local_ner_client.LocalNERHandler


def load_all_resources() -> None:
    """무거운 리소스를 프로세스당 1회 로드한다 (FastAPI lifespan에서 호출)."""
    global PARSER, KIWI, GLOSS_DICT, CE_TOK, CE_MODEL, AI_HANDLER, NNP_HANDLER
    ...
    print(f"[pipeline] NNP 핸들러 로딩 중... (backend={NNP_BACKEND})")
    if NNP_BACKEND == "local":
        NNP_HANDLER = local_ner_client.LocalNERHandler()
    else:
        NNP_HANDLER = gemini_client.GeminiHandler(GEMINI_API_KEY)
...
def run_analysis(text: str) -> dict:
    ...
    # 고유명사(NNP) 판별: Kiwi 분석 전에 (Gemini 또는 로컬 NER로) 스팬을 뽑아
    # pretokenized로 고정. 실패해도 빈 리스트라 원본 Kiwi 분석 그대로 진행됨
    # (파이프라인 안 막힘).
    nnp_spans = NNP_HANDLER.detect_nnp(text)
